...
\cref{lem:gradient_coupling} implies that ReLU signs will not change after finite policy gradient updates, i.e., $\sI\{ \rvw_r(t)^\top \rvs > 0 \} = \sI\{ \rvw_r(0)^\top \rvs > 0 \}$. Combine \cref{lem:logit_smoothness} with \cref{lem:gradient_coupling}, we have smoothness property in parameter space.
\begin{lem}
...
\textbf{\cref{thm:logit_learning_main_result}.} Given value neural networks as shown in \cref{fig:nn_policy_value}, with number of hidden nodes $m \ge T^2 / K^2$, learning rate $\eta = \frac{1}{K m}$,  $\alpha > 3$, and $\beta > 256$, the expected regret of \cref{alg:logit_learning_eps_greedy_exploration} satisfies,
\begin{equation*}
\begin{split}
